...
- (IBAction)SendC4:(UIButton *)sender {
    [self midiSendNoteOnInForeground:41];
}
- (IBAction)SendE4:(UIButton *)sender {
    [self midiSendNoteInForeground:45];
}
- (IBAction)SendCC:(UIButton *)sender {
    [self midiSendCCInForeground:33];

}

- (IBAction)ReleaseC4:(UIButton *)sender {
    [self midiSendNoteOffInForeground:41];
}
...
- (void) midiSendNoteOnInForeground:(UInt8)n
{
    const UInt8 note      = n;
    const UInt8 noteOn[]  = { 0x90, note, 127 };
...
- (void) midiSendNoteOffInForeground:(UInt8)n
{
    const UInt8 note      = n;
    const UInt8 noteOff[]  = { 0x80, note, 0 };
...
- (void) midiSendCCInForeground:(UInt8)n
{
    const UInt8 note      = n;
    const UInt8 noteCC[]  = { 0xB0, note, 65 };
    
    [midi sendBytes:noteCC size:sizeof(noteCC)];
}
...
-(void) DesignStuff:(UITableViewCell* ) cell{ // you can safely delete this but then you also have to delete the method call above.
    if(rows%2==0)
        cell.backgroundColor=[UIColor colorWithRed:(CGFloat)1.0 green:(CGFloat)0.0 blue:(CGFloat)0.0 alpha:(CGFloat)1.0];
    else
        cell.backgroundColor=[UIColor colorWithRed:(CGFloat)1.0 green:(CGFloat)0.2 blue:(CGFloat)0.2 alpha:(CGFloat)1.0];
    rows++;
    
    cell.textLabel.textColor=[UIColor whiteColor];
    [cell.textLabel setFont:[UIFont fontWithName:@"Arial-BoldMT" size:18.0f]];
}
